...
const positionColors = {
  ...
  'Senior Agent': "bg-purple-100 text-purple-800 dark:bg-purple-900/20 dark:text-purple-400",
  'Team Lead': "bg-orange-100 text-orange-800 dark:bg-orange-900/20 dark:text-orange-400",
...
const certificationStatusColors = {
  ...
  'Pending Renewal': "bg-yellow-100 text-yellow-800 dark:bg-yellow-900/20 dark:text-yellow-400",
};
...
export default function Agents() {
  ...
  const agentColumns: ColumnDef<Agent>[] = [
    ...
    },
    {
      ...
      header: "Position",
      cell: ({ row }) => {
        const position = row.getValue("position") as keyof typeof positionColors;
        return (
          <Badge className={positionColors[position]}>
            {position}
          </Badge>
        );
      },
      ...
      cell: ({ row }) => {
        ...
        return (
          <div>
            ...
            <div className="text-sm text-muted-foreground">{agent.territory.customerCount} customers</div>
          </div>
        );
      },
    },
        ...
        return (
          <div className="flex items-center space-x-2">
            <div className="flex items-center">
              ...
              <span className="font-medium">{agent.performance.rating.toFixed(1)}</span>
            </div>
  ...
  const totalCommission = agents.reduce((sum, agent) => sum + agent.performance.yearToDate.commissionEarned, 0);
  const totalPremium = agents.reduce((sum, agent) => sum + agent.performance.yearToDate.premiumGenerated, 0);
  const averageRating = agents.reduce((sum, agent) => sum + agent.performance.rating, 0) / agents.length;
  const expiringCerts = getExpiringCertifications(30);

  // Team performance for team leads
  const teamLeads = agents.filter(agent => agent.position === 'Team Lead' && agent.teamMembers);
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Leading agents by performance rating</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  {topPerformers.map((agent, index) => (
                    <div key={agent.id} className="flex items-center justify-between p-4 border rounded-lg dark:border-gray-700">
                      ...
                      <div className="text-right">
                        ...
                        <div className="text-sm text-muted-foreground">
                          {formatCurrency(agent.performance.yearToDate.premiumGenerated)}
                        </div>
                      </div>
                    </div>
                  ))}
                ...
                <div className="space-y-6">
                  <div>
                    <div className="flex justify-between text-sm mb-2">
                      ...
                      <span>{agents.reduce((sum, agent) => sum + agent.performance.yearToDate.policiesSold, 0)}</span>
                    </div>
                  ...
                  <div>
                    <div className="flex justify-between text-sm mb-2">
                      ...
                      <span>{formatPercentage(agents.reduce((sum, agent) => sum + agent.performance.yearToDate.customerRetention, 0) / agents.length)}</span>
                    </div>
                  ...
                  <div>
                    <div className="flex justify-between text-sm mb-2">
                      ...
                      <span>{formatPercentage(agents.reduce((sum, agent) => sum + agent.performance.yearToDate.conversionRate, 0) / agents.length)}</span>
                    </div>
                  ...
                  <div>
                    <div className="flex justify-between text-sm mb-2">
                      ...
                      <span>{(agents.reduce((sum, agent) => sum + agent.performance.yearToDate.customerSatisfaction, 0) / agents.length).toFixed(1)}/5.0</span>
                    </div>
                ...
                <Card key={teamLead.id}>
                  <CardHeader>
                    <CardTitle className="flex items-center space-x-2">
                      ...
                      <span>{teamLead.firstName} {teamLead.lastName}'s Team</span>
                    </CardTitle>
                    <CardDescription>
                      {teamMetrics.teamSize} team members • {teamLead.territory.name}
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="space-y-2">
                          <div className="flex justify-between text-sm">
                            ...
                            <span className="font-medium">{teamMetrics.totalPolicies}</span>
                          </div>
                          <div className="flex justify-between text-sm">
                            <span>Total Premium:</span>
                            <span className="font-medium">{formatCurrency(teamMetrics.totalPremium)}</span>
                          </div>
                          <div className="flex justify-between text-sm">
                            <span>Average Rating:</span>
                            <span className="font-medium">{teamMetrics.averageRating.toFixed(1)}</span>
                          </div>
                        </div>
                      </div>
                        ...
                        <div className="space-y-3">
                          <div>
                            <div className="flex justify-between text-sm mb-1">
                              <span>Policies</span>
                              <span>{formatPercentage(targetProgress.policies.percentage)}</span>
                            </div>
                            <Progress value={targetProgress.policies.percentage} className="h-2" />
                          </div>
                          <div>
                            <div className="flex justify-between text-sm mb-1">
                              <span>Premium</span>
                              <span>{formatPercentage(targetProgress.premium.percentage)}</span>
                            </div>
                            <Progress value={targetProgress.premium.percentage} className="h-2" />
                          </div>
                        </div>
                      </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Latest training completions and certifications</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                    ...
                    return (
                      <div key={record.id} className="flex items-center justify-between p-3 border rounded-lg dark:border-gray-700">
                        <div>
                          ...
                          <div className="text-sm text-muted-foreground">
                            {agent?.firstName} {agent?.lastName} • {formatDate(record.completionDate || record.startDate)}
                          </div>
                        </div>
                        <div className="text-right">
                          <Badge className={cn(
                            record.status === 'Completed' && "bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-400",
                            record.status === 'In Progress' && "bg-blue-100 text-blue-800 dark:bg-blue-900/20 dark:text-blue-400",
                            record.status === 'Failed' && "bg-red-100 text-red-800 dark:bg-red-900/20 dark:text-red-400"
                          )}>
                            {record.status}
                          </Badge>
                          {record.score && (
                ...
                <div className="space-y-4">
                  {expiringCerts.length > 0 ? (
                    expiringCerts.map(({ agent, certification }) => (
                      <div key={`${agent.id}-${certification.id}`} className="flex items-center justify-between p-3 border rounded-lg dark:border-gray-700 border-yellow-200 bg-yellow-50 dark:bg-yellow-900/10">
                        <div>
      ...
      <FormDialog
        ...
        title={selectedAgent ? `${selectedAgent.firstName} ${selectedAgent.lastName}` : "Agent Details"}
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-1">
                      <Star className="h-4 w-4 text-yellow-500" />
                      <span className="font-medium">{selectedAgent.performance.rating.toFixed(1)}</span>
                    </div>
                  </div>
                  <div className="flex items-center justify-between">
                    <span>Rank:</span>
                    <span>#{selectedAgent.performance.rank} of {selectedAgent.performance.totalAgents}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span>YTD Premium:</span>
                    <span className="font-medium">{formatCurrency(selectedAgent.performance.yearToDate.premiumGenerated)}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span>Commission:</span>
                    <span className="font-medium">{formatCurrency(selectedAgent.performance.yearToDate.commissionEarned)}</span>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <Card>
                  <CardContent className="p-4 text-center">
                    <div className="text-2xl font-bold text-blue-600">{selectedAgent.performance.yearToDate.policiesSold}</div>
                    <div className="text-sm text-muted-foreground">Policies Sold</div>
                  </CardContent>
                </Card>
                <Card>
                  <CardContent className="p-4 text-center">
                    <div className="text-2xl font-bold text-green-600">{formatPercentage(selectedAgent.performance.yearToDate.customerRetention)}</div>
                    <div className="text-sm text-muted-foreground">Retention Rate</div>
                  </CardContent>
                </Card>
                <Card>
                  <CardContent className="p-4 text-center">
                    <div className="text-2xl font-bold text-purple-600">{formatPercentage(selectedAgent.performance.yearToDate.conversionRate)}</div>
                    <div className="text-sm text-muted-foreground">Conversion Rate</div>
                  </CardContent>
                </Card>
                <Card>
                  <CardContent className="p-4 text-center">
                    <div className="text-2xl font-bold text-yellow-600">{selectedAgent.performance.yearToDate.customerSatisfaction.toFixed(1)}</div>
