...
class MockDatabase {
  ...
  getDashboardMetrics() {
    return {
      ...
      activePolicies: this.data.policies.filter((p) => p.status === "Active")
        .length,
      totalCustomers: this.data.customers.length,
      totalClaims: this.data.claims.length,
      pendingClaims: this.data.claims.filter((c) => c.status === "Pending")
        .length,
      totalPremiums: this.data.policies.reduce(
        (sum, p) => sum + p.premiumAmount,
        0,
      ),
      totalCoverage: this.data.policies.reduce(
...
export const apiRoutes = {
  ...
  "GET /api/customers": async (params: any) => {
    ...
    if (search) {
      customers = db.search("customers", search, [
        "firstName",
        "lastName",
        "email",
        "phone",
    ...
    const paginatedData = db.paginate(
      customers,
      parseInt(page),
      parseInt(pageSize),
    );
  ...
  "GET /api/policies": async (params: any) => {
    ...
    if (search) {
      policies = db.search("policies", search, [
        "policyNumber",
        "customerName",
        "policyType",
      ]);
  ...
  "POST /api/policies": async (params: any) => {
    ...
    if (!body.customerName || !body.policyType || !body.premiumAmount) {
      return createErrorResponse(
        "Missing required fields: customerName, policyType, premiumAmount",
      );
    }
  ...
  "GET /api/claims": async (params: any) => {
    ...
    if (search) {
      claims = db.search("claims", search, [
        "claimNumber",
        "customerName",
        "description",
      ]);
  ...
  "POST /api/claims": async (params: any) => {
    ...
    if (!body.customerName || !body.policyId || !body.amount) {
      return createErrorResponse(
        "Missing required fields: customerName, policyId, amount",
      );
    }
  ...
  "GET /api/ai/insights": async () => {
    ...
    return createResponse({
      ...
      predictions: [
        {
          type: "Churn Risk",
          value: Math.random() > 0.5 ? "Low" : "Medium",
        ...
        {
          type: "Claims Spike",
          value: Math.random() > 0.7 ? "High" : "Moderate",
...
export function setupDummyAPI() {
  ...
  window.fetch = async (
    ...
    if (url.startsWith("/api") || url.includes("/api/")) {
      ...
      console.log(
        `[Dummy API] Looking for route: ${routeKey}, found: ${!!handler}`,
      );
      ...
      if (!handler) {
        for (const [pattern, routeHandler] of Object.entries(apiRoutes)) {
          ...
          if (
            ...
            isRouteMatch(patternPath, urlPath || url)
          ) {
