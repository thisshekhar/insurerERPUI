...
export interface Agent {
  ...
  department: 'Sales' | 'Underwriting' | 'Claims' | 'Customer Service' | 'Operations';
  territory: Territory;
  address: Address;
...
export interface AgentActivity {
  ...
  agentId: string;
  date: string;
  type: 'Call' | 'Meeting' | 'Email' | 'Quote' | 'Sale' | 'Follow-up' | 'Training';
  description: string;
  customerId?: string;
  policyId?: string;
  outcome: 'Successful' | 'Follow-up Required' | 'No Interest' | 'Scheduled' | 'Completed';
...
export const mockAgents: Agent[] = [
  {
    ...
    commissionStructure: {
      ...
      tierRates: [
        ...
        { threshold: 250000, rate: 6.0, description: "Enhanced rate $100K-$250K" },
        { threshold: 500000, rate: 7.0, description: "Premium rate $250K-$500K" },
        { threshold: 1000000, rate: 8.0, description: "Elite rate above $500K" }
      ],
      bonuses: [
        {
          id: "BON-001",
    ...
    notes: "Top performing agent with excellent customer satisfaction scores. Strong in life insurance sales.",
    manager: "John Smith"
  },
  {
    id: "AGT-002",
    ...
    notes: "Promising new agent with strong digital sales skills. Focusing on auto insurance market.",
    manager: "John Smith"
  },
  {
    id: "AGT-003",
    ...
    notes: "Experienced agent with excellent retention rates. Specializes in health insurance and complex cases.",
    manager: "John Smith"
  },
  {
    id: "AGT-004",
    ...
    notes: "New agent in training program. Showing good progress and potential. Requires mentoring support.",
    manager: "John Smith"
  },
  {
    id: "AGT-005",
    ...
    notes: "Exceptional team leader and top performer. Specializes in commercial insurance and mentors junior agents.",
...
export const mockAgentActivities: AgentActivity[] = [
  {
    ...
    date: "2024-02-20",
    type: "Call",
    description: "Follow-up call with Alice Johnson regarding life insurance renewal",
    customerId: "CUST-001",
    policyId: "POL-001",
    outcome: "Successful",
    duration: 25,
  ...
  },
  {
    ...
    date: "2024-02-19",
    type: "Meeting",
    description: "Initial consultation with potential customer for auto insurance",
...
export const calculateTeamMetrics = (teamLeadId: string) => {
  ...
  return {
    totalPolicies: allTeamMembers.reduce((sum, agent) => sum + agent.performance.yearToDate.policiesSold, 0),
    totalPremium: allTeamMembers.reduce((sum, agent) => sum + agent.performance.yearToDate.premiumGenerated, 0),
    totalCommission: allTeamMembers.reduce((sum, agent) => sum + agent.performance.yearToDate.commissionEarned, 0),
    averageRating: allTeamMembers.reduce((sum, agent) => sum + agent.performance.rating, 0) / allTeamMembers.length,
    averageRetention: allTeamMembers.reduce((sum, agent) => sum + agent.performance.yearToDate.customerRetention, 0) / allTeamMembers.length,
...
export const getExpiringCertifications = (daysFromNow: number = 30): { agent: Agent; certification: Certification }[] => {
...
export const calculateTeamTargetProgress = (teamLeadId: string) => {
  ...
  const totalTargets = allTeamMembers.reduce((acc, agent) => {
  ...
  const totalActual = allTeamMembers.reduce((acc, agent) => {
