...
export const generateAgentPerformanceData = (count: number = 10): AgentPerformanceData[] => {
  const firstNames = ['Mike', 'Sarah', 'James', 'Lisa', 'David', 'Emma', 'Chris', 'Amy', 'Robert', 'Jennifer'];
  const lastNames = ['Chen', 'Williams', 'Brown', 'Anderson', 'Kim', 'Davis', 'Johnson', 'Wilson', 'Garcia', 'Miller'];
...
export const generateClaimsData = (): ClaimsData => {
  const byType: ClaimsByType[] = [
    { type: 'Auto', claims: 140, approved: 125, avgAmount: 8500, processingTime: 7.5 },
    { type: 'Health', claims: 89, approved: 81, avgAmount: 3200, processingTime: 5.2 },
    { type: 'Life', claims: 45, approved: 42, avgAmount: 25000, processingTime: 12.1 },
    { type: 'Home', claims: 38, approved: 32, avgAmount: 12000, processingTime: 9.8 },
    { type: 'Business', claims: 28, approved: 18, avgAmount: 45000, processingTime: 15.3 }
  ];
  ...
  const averageProcessingTime = byType.reduce((sum, t) => sum + t.processingTime * t.claims, 0) / totalClaims;
  const totalPayout = byType.reduce((sum, t) => sum + t.approved * t.avgAmount, 0);
...
export const generateCustomerMetrics = (): CustomerMetrics => {
  const byAgeGroup: CustomerAgeGroup[] = [
    { group: '18-25', count: 89, percentage: 7.1, averagePremium: 1800, retentionRate: 88.5 },
    { group: '26-35', count: 312, percentage: 25.0, averagePremium: 2400, retentionRate: 92.1 },
    { group: '36-45', count: 398, percentage: 31.9, averagePremium: 3200, retentionRate: 95.8 },
    { group: '46-55', count: 267, percentage: 21.4, averagePremium: 3800, retentionRate: 96.2 },
    { group: '56-65', count: 134, percentage: 10.7, averagePremium: 4200, retentionRate: 97.1 },
    { group: '65+', count: 47, percentage: 3.8, averagePremium: 3600, retentionRate: 94.3 }
  ];
  ...
  const weightedRetention = byAgeGroup.reduce((sum, g) => sum + g.retentionRate * g.count, 0) / totalCustomers;
  const lifetimeValue = byAgeGroup.reduce((sum, g) => sum + g.averagePremium * g.count, 0) / totalCustomers * 5.2;
...
export const generateFinancialMetrics = (): FinancialMetrics => {
  ...
  const profit = totalPremium - totalClaims - operatingExpenses + investmentIncome;
...
export const getHealthColor = (value: number, threshold: number = 80): string => {
