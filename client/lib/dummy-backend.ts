...
class DummyDatabase {
  ...
  getDashboardMetrics() {
    return {
      ...
      totalCoverage: this.policies.reduce(
        (sum, p) => sum + p.coverageAmount,
        0,
      ),
      claimsAmount: this.claims.reduce((sum, c) => sum + c.amount, 0),
      avgPolicyValue:
        this.policies.reduce((sum, p) => sum + p.premiumAmount, 0) /
  ...
  getFinancialReport(dateRange?: any) {
    ...
    return {
      revenue: {
        totalPremiums: policies.reduce((sum, p) => sum + p.premiumAmount, 0),
        paidPremiums: payments
          .filter((p) => p.status === "Paid")
          .reduce((sum, p) => sum + p.amount, 0),
        pendingPremiums: payments
          .filter((p) => p.status === "Pending")
          .reduce((sum, p) => sum + p.amount, 0),
        overduePremiums: payments
  ...
  private getRiskAnalysis() {
    return {
      ...
      mediumRisk: this.policies.filter((p) => p.riskAssessment === "Medium")
        .length,
  ...
  "GET /api/riders/policy-type/:type": (params: any) => {
    const riders =
      availableRidersByPolicyType[
        params.type as keyof typeof availableRidersByPolicyType
      ] || [];
